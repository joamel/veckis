      ...
      <View style={s.body}>
        ...
        {!loading && events && events.length > 0 && (
          <Pressable style={s.refreshBtn} onPress={load} hitSlop={6}>
            <Ionicons name="refresh-outline" size={14} color={nyDesign ? ny.padYta : c.textMuted} />
...
const makeStyles = (c: Palette, nyD: boolean, ny: NyPalett) => StyleSheet.create({
  ...
  title: nyD
    ? { flex: 1, fontFamily: nyFont.halvfet, fontWeight: 'normal', fontSize: 15, color: ny.padYta }
  ...
  refreshText: { fontSize: 12, color: nyD ? ny.padYta : c.textMuted },
});
