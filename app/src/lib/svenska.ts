...
export const shoppingList = {
  ...
  importera: {
    titel:           'Importera varor',
    valUnder:        'Från en lapp, en skärmbild eller en annan lista',
    kamera:          'Fota listan',
    kameraHint:      'Handskriven lapp eller tryckt lista',
    bild:            'Välj bild',
    bildHint:        'Foto eller skärmbild från galleriet',
    text:            'Klistra in text',
    textHint:        'Kopiera från Keep, Anteckningar, ett sms …',
    textPlaceholder: 'En vara per rad, t.ex.\nmjölk 2 l\n500 g köttfärs\nbröd',
    tolka:           'Fortsätt',
    laser:           'Läser av listan …',
    granskaTitel:    (n: number) => `${n} varor hittade`,
    granskaUnder:    'Kryssa ur det du inte vill lägga till. Namn som matchats mot era varor står med originalet under.',
    laggTill:        (n: number) => n === 1 ? 'Lägg till 1 vara' : `Lägg till ${n} varor`,
    tillagda:        (n: number) => n === 1 ? '1 vara tillagd' : `${n} varor tillagda`,
    ingaVaror:       'Hittade inga varor att lägga till',
    kapad:           (n: number) => `Listan var längre än taket — de ${n} första varorna visas`,
    frånNamn:        (original: string) => `från "${original}"`,
    matchadTill:     (matchad: string) => `matchad som "${matchad}"`,
    antalRader:      (n: number) => `står ${n} gånger`,
    behållOriginal:  'Använd namnet som stod i listan',
    användMatchad:   'Använd hushållets namn',
    laggerTill:      (klara: number, totalt: number) => `Lägger till ${klara}/${totalt} …`,
    fotoMisslyckades:     'Kunde inte läsa av bilden',
    textMisslyckades:     'Kunde inte läsa texten',
    laggTillMisslyckades: 'Kunde inte lägga till varorna',
  },
...
export const recipes = {
  ...
  createModal: {
    ...
    photo: {
      ...
      removePage:       (n: number) => `Ta bort sida ${n}`,
      pagesHint:        'Fota eller välj flera sidor om receptet inte får plats på en. Ordningen spelar roll.',
      maxPages:         (max: number) => `Max ${max} sidor per recept.`,
      stageAnalyzingMulti: (n: number) => `Analyserar ${n} sidor…`,
      multiTitle:       (n: number) => `Hittade ${n} recept`,
      multiMessage:     'Kryssa i de recept du vill spara.',
      multiSummary:     (n: number) => `${n} ingredienser`,
