...
- [x] 🐛 **Standardikonen för ny inköpslista matchar inte den faktiska ikonen** — löst enklare (2026-09-07, efter feedback): ingen placeholder-emoji alls i `EmojiPicker`s textfält längre, i stället för att bara byta ut den mot en "rättare" gissning (🛒) — vald/ej vald syns redan tydligt via vilken förslags-chip som är markerad, en ikon-"förhandsvisning" i fältet la bara till en (potentiellt missvisande) extra sanningskälla.
- [x] 🐛 **Kategori-auto-scroll funkade inte när man valde en vara från sökförslagen** — löst: den gamla "onLayout bara på den aktiva chippen"-tekniken (`stores/[storeId].tsx`s mönster, återanvänt i `shopping/[listId].tsx`) fungerar bara vid FÖRSTA layouten — satte man kategori programmatiskt (tryck på ett sökförslag) i en picker som redan legat monterad sen tidigare kom inget nytt onLayout-anrop, så raden stod kvar längst till vänster. Ny gemensam `useChipAutoScroll`-hook sparar ALLA chippars x-position (inte bara den aktiva) och skrollar i en effekt som körs varje gång vald kategori ändras — funkar oavsett om kategorin sattes via tryck i pickern eller programmatiskt. Applicerad på redigera-vara, lägg till vara OCH stapel-/sökförslags-redigeraren (som helt saknade auto-scroll sedan tidigare).
