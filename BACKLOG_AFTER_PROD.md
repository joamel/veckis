...
- [x] 🐛 **Standardikonen för ny inköpslista matchar inte den faktiska ikonen** — löst: `EmojiPicker`s tomma-fält-placeholder visade 🧹 (sopkvast) men den RIKTIGA fallback-ikonen på ett listkort utan emoji är Ionicons `cart-outline`. Placeholder bytt till 🛒 så den matchar det man faktiskt får.
