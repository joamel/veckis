...
- [x] Radera mitt konto-knapp i Profil → ÖVRIGT: röd "Ta bort kontot"-rad med confirm-dialog som öppnar Clerks user-profile (samma in-app-browser som 2FA-länken via openClerkPortal-helper) där användaren kan radera Clerk-kontot. Backend bör senare fånga Clerks user.deleted-webhook och rensa hushållsmedlemskap — kvarstår som framtida punkt.
- [ ] Backend: Clerk user.deleted-webhook → ta bort/föräldralös hushållsmedlemskap. Idag öppnar "Ta bort kontot" Clerks portal där användaren raderar sitt konto, men backendens HouseholdMember-rader med dess clerkUserId blir kvar (orphaned). Med webhook: rensa medlemskap, om hen är ende admin → markera hushållet för städning eller hand över till annan medlem.
...
- [x] "Du handlar nu" bannern lägger sig ovanpå rubriken: bannern var absolut-positionerad i titel-områdets position. Först flyttad till en centrerad pill i navbaren, men den krockade med rubriken när den fälls upp till mitten vid scroll. Slutlig lösning: en rosa gubbe-ikon (walk) till höger om navbaren (vänster om 3-prickar), tryckbar → toast med vem som handlar. Krockar inte i något scroll-läge.
...
- [x] Klickar man på en aktivitet borde man inte hamna direkt i redigeringsläge utan bara read-vy med sammanfattning + redigering under 3 prickar: ny `viewingEntry`-läsvy (modal) som visar titel + datum/veckodag, tid/heldag, upprepning (ny `recurrenceSummary`-helper), tilldelade personer, gemensam/privat och beskrivning. 3-prickar i headern (`openEntryActions`) → Redigera/Ta bort. Tap på kortet öppnar nu läsvyn (long-press → 3-prickar-menyn direkt); tablet-månadsvyn öppnar också läsvyn. (Testfix: läsvyn klipptes nedtill — lade till safe-area-bottenmarginal via `useSafeAreaInsets`.)
- [x] Aktivitetsdialogen (testfynd): (a) ingen toast vid spara/skapa — nu `showToast('Aktivitet sparad'/'skapad', 'success')`; (b) gick inte att trycka på andra knappar (t.ex. tilldela personer) med tangentbordet uppe — `keyboardShouldPersistTaps="handled"` på edit- och create-modalernas ScrollViews.
