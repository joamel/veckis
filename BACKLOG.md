...
- [ ] Onboarding feedback: Första gången borde det stå typ "Välkommen till Veckis, här följer lite tips och trix - fortsätt/avbryt" samt en checkbox för att "Jag vill inte ha fler tips"/"jag är fullärd" typ för hela appen; all text syns inte i tipsen; Ljusa fältet borde ha samma border som pulserande ringen; man ser igenom highlightingen vilket är förvirrande om det är text bakom; borde vara animering för swiping och att byta plats på rätter, tryck på veckonummer borde bara highlighta själva veckonumret; menyn borde ge tips om var maträtter och sysslor kommer ifrån samt att "+" för att skapa aktiviteter (återkommande, vem osv); första tipset i sysslor borde vara vad man ska med fliken till typ "Här kan du strukturera upp återkommande sysslor, eller varför inte prova roterande schema så att alla kan turas om samt klarmarkera" typ; Inga fler påminnelser borde stå när man fäller ut sysslan men kanske beskriva det mer som "Här ser du historik över återkommande sysslor (klar/missad) etc"; sortera-tipset är onödigt då man kanske inte har några recept inlagda, bätte att ge tips när man trycker på "+" att man kan skapa eget recept eller importera med en URL;
...
- [ ] Flera medlemmar per syssla + valbar rotation: ersätt enstaka `assignedTo` med `assignedMemberIds: string[]` + `rotation: boolean`-flagga. **rotation=false** = alla i listan är gemensamt ansvariga (symmetri-gap mot aktiviteters `assignedToMany`). **rotation=true** = de turas om per tillfälle, kalender-låst: `turn = list[occurrenceIndex % list.length]` (deterministiskt från `startDate`/`createdAt`, missade tillfällen flyttar *inte* turen). Kortet visar "Annas tur" + ev. "Nästa: Bo". Performer-pickern öppnas med tur-personen överst men kan alltid överstyras. Kan byggas inkrementellt: börja med multi-assign (rotation=false default), lägg till `rotation`-flagga ovanpå.
