...
- [ ] Flera medlemmar per syssla + valbar rotation (story-headline; sub-punkter nedan). **rotation=false** = alla i listan är gemensamt ansvariga (symmetri-gap mot aktiviteters `assignedToMany`). **rotation=true** = de turas om per tillfälle, kalender-låst: `turn = list[occurrenceIndex % list.length]` (deterministiskt från `startDate`/`createdAt`, missade tillfällen flyttar *inte* turen).
- [ ] Sysslor – datamodell: Prisma-migration `Chore.assignedTo: String?` → `assignedMemberIds: String[]` + `rotation: Boolean default false`. Migrera befintliga rader (`'abc'` → `['abc']`, `null` → `[]`). Uppdatera create/update-endpoints + zod-schema i `shared/`.
- [ ] Sysslor – backend-logik: notification-fan-out (push till alla i listan vid `rotation=false`, bara tur-personen vid `rotation=true`). Filter-query matchar `memberId IN assignedMemberIds`. Hjälpare `computeCurrentTurn(chore, occurrenceIndex)` används både i notis-jobbet och i API-svaret så frontend slipper räkna.
- [ ] Sysslor – editor: ersätt single-select tilldelad-medlem med multi-select chips (lokala profiler + Clerk-medlemmar). Toggle "Turas om" visas när 2+ är valda. Inline-mikroforklaring av rotation-läget.
- [ ] Sysslor – syssla-kort: avatar-stack (max 3 + "+N") istället för en avatar. `rotation=true` → "Annas tur · Nästa: Bo". `rotation=false` → "Anna · Bo".
- [ ] Sysslor – performer-picker: vid `rotation=true` förvalt tur-personen överst, men hela tilldelade listan + lokala profiler valbara så man kan överstyra (någon annan hoppade in).
- [ ] Sysslor – filter-konsistens: "Mina sysslor" + medlemsfilter matchar nu `memberId IN assignedMemberIds` (idag jämför vi mot enstaka `assignedTo`). Symmetri med kalenderns aktivitets-filter.
- [ ] Sysslor – edge cases: borttagen medlem rensas från alla sysslors arrays (tom array = ingen specifik tilldelning, samma semantik som dagens `assignedTo=null`). Lokala profiler behåller passiv-roll. Orphanvarningen vid medlem-borttagning räknar nu "sysslor där bara personen var tilldelad".
- [ ] Sysslor – realtidsuppdatering av rotation: när någon klarmarkerar en roterande syssla flyttas turen, andra enheter ser det direkt via befintlig `chore_updated`-socket. Backend skickar med ny `currentTurnMemberId` i payload så frontend slipper omräkna lokalt.
- [ ] Sysslor – historik visar tur-vs-utförare: utfälld historik markerar både vems tur det var OCH vem som faktiskt klarade av den. Visuellt särskilt när de skiljer sig (någon hoppade in).
- [ ] Sysslor – "min tur"-snabbfilter: toggle i sysslor-filtret som växlar mellan "alla där jag är med" och "bara där jag har tur just nu". Praktiskt för "vad ska JAG göra idag?".
- [ ] Sysslor – rotation-action-tip: när användaren först väljer en andra medlem i sysslan, fyrar ett kort tip vid rotation-toggle:n: "Slå på Turas om så turas ni automatiskt. Lämnas av = alla är gemensamt ansvariga."
