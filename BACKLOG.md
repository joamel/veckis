...
- [x] PWA: gick inte att scrolla i inköpslistan — swipe-delete-gestens GestureDetector satte touch-action:none på varje varurad (samma rotorsak som veckomeny-korten). Web renderar nu raderna utan gest + ✕-delete-knapp (stilen fanns redan); InvSlider i inventeringen hade samma problem → tap-to-set via Pressable på web. Audit: övriga GestureDetector-ytor (maträttskort) redan gatade; push/SecureStore/haptik har web-varianter
