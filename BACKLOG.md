...
- [x] Tomma dagar i veckomenyn är lägre än dagar med inlagda maträtter — inkonsekvent höjd ger konstigt UX, alla dagslotter bör ha fast/konsekvent minhöjd: `daySlotEmpty.minHeight` 44→64 matchar nu en fylld dags kortshöjd
...
- [x] Offline-synk för sysslor — avbocker man en syssla utan nät tappas det tyst. Kö-mönstret från `shoppingOfflineQueue` kan återanvändas: mutationer köas lokalt och spolas upp mot API:et vid nästa anslutning.
...
- [x] "Fler inställningar"-sektion i syssla-editorn: dölj slutdatum och rotationsordning bakom en expand-knapp. Grundflödet (titel + upprepning + dag + person) räcker för 90 % av sysslorna — avancerade fält finns men tränger inte på.
- [x] Rotationsordning-editorn dold tills den är meningsfull: visa den bara när rotation är på *och* 3+ medlemmar är valda. Med 2 medlemmar är ordningen självklar (turvis), ingen editor behövs.
- [x] UX-pass sysslo-editorn (omgång 2): upprepningspilarna bredvid rubriken (flexShrink på titel, flexShrink:0 på ikon), X-knapp i datum- och upprepningsrutorna, startdatum ovanför upprepning, "Fler inställningar"-toggle borttagen, "Turas om" alltid synlig för 2+ medlemmar och öppnar dedikerad turordningsmodal (centrerat kort, animationType="fade") i stället för inline-lista
- [x] Syssla-kortets meta-text visar bara aktuell turperson ("Annas tur") — ingen "Nästa: X"-suffix (buildAssignedLabel.ts)
- [x] "Vill du slänga utkastet?"-dialog vid avbrytning av osparade formulär: useDiscardDraft-hook (confirm → "Vill du slänga utkastet?" / "Fortsätt redigera" / "Släng utkast") i chores (create + edit), schedule (create + edit), shopping (create), recipes (create + edit), stores (create)
- [x] Turordningsmodal i edit-läget: onOpenOrderModal-propen finns bara i create-flödet — redigerar man en sparad syssla med rotation kan man inte öppna turordningsmodalen för att justera ordningen
- [x] Tre-punkts-menyn (variant:'menu') visar ingen rubrik och har ikon per rad: `icon?: string` på ConfirmButton, Ionicons renderas till vänster om label-texten, `title` optional i ConfirmOptions. Alla anrop uppdaterade (sysslor, aktiviteter, recept, butiker) med passande Ionicons-namn + title-fältet borttaget
- [x] Sysslors tre-punkts-meny: Redigera ligger nu före Kopiera (logisk prioritetsordning)
- [x] Kalenderkortet för maträtter visar skalad portionsmängd: `item.servings ?? item.recipe.servings` i stället för `item.recipe.servings` — respekterar skalning satt i meny-vyn
- [x] Swipe-to-delete i inköpslistan fungerar nu i PWA: `!isWeb`-guard borttagen ur `panGesture.enabled()` — RNGH Pan-gestyren med `activeOffsetX` + `failOffsetY` kör på alla plattformar; separat web-gren med soptunna-knapp borttagen
- [x] Inköpslistans 3-prickar-meny: ikoner och text lila (`#4f46e5`) — `actionsMenuText.color` + de tidigare lila-lila (`#7c3aed`) ikonerna (shopper, dubbletter) uppdaterade; röda destructiva-poster oförändrade
...
- [x] Sysslor: 4+2 buggar. (1+2) Historik/state-reset vid refresh: `completions: { take: 1 }` i backend gav bara 1 completion → borttagen. (3) Syssla med passerat slutdatum låg kvar i listan: skip om state='none' och nextDate=null; nextDate nollställs om det överstiger endDate. (4) completedDate visade äldsta istället för senaste: sätts nu till senast avbockad occurrence. (5) Rensade sysslor återkom vid refresh: count-jämförelsen var baserad på gamla take:1-värdet (alltid 1) och matchade inte längre; ersatt med enbart state='done'-check. (6) Framtida pre-completade syns inte i historiken: framtida completions läggs nu till i occurrences-arrayen och sorteras in kronologiskt.
...
- [ ] Recept-taggar/samlingar — märk recept som "vegetariskt", "snabbt", "barnvänligt", "favorit" m.m. och filtrera på dessa i receptlistan. Naturligt nästa discovery-steg när hushållet har 30+ recept och sökning på namn/ingrediens inte räcker för att hitta rätt.
...
- [ ] **Rate limiting** — Inga rate limits på backend-endpoints. Minst tre ytor behöver skydd: `POST /api/recipes/fetch-url` (extern URL-fetch = SSRF-yta), `POST /api/recipes/parse-text` (dyr AI-körning), push-token-registrering (spamrisk). `express-rate-limit` per IP räcker som första steg.
