...
- [x] Onboarding-tips för dolda funktioner via ny `useOnceFlag`-hook (expo-secure-store) + `ConfirmDialog` — en flagga per moment, kan stackas över tid. Klart hittills: (1) `seen-forgiving-tip` — förfallna sysslor påminner inte mer; (2) `seen-menu-nav-tip` — dra rätter mellan dagar + svep mellan veckor; (3) `seen-merge-tip` — vad dubblett-knappen är när den poppar upp första gången. Markeras som sedda vid 'Förstått'-tryck.
