...
- [x] Onboarding-tips för dolda funktioner via `useOnceFlag`-hook + `SpotlightTip`-komponent: dimmar bakgrunden och ringer in mål-elementet med en pulserande ram (eller centrerad ruta om inget mål). `showTip` returnerar boolean — om ett annat tip redan visas blir det `false` och `markSeen` skippas så tipset försöks igen senare (förhindrar att tips trampar på varandra). Tips visas en gång per device. Sju tips klara: (1) `seen-forgiving-tip` förfallna sysslor påminner inte mer (sysslor); (2) `seen-menu-nav-tip` drag + svep (meny); (3) `seen-merge-tip` dubblett-knappen (inköp, target); (4) `seen-templates-tip` veckomeny-mallar (meny, target, efter nav-tip); (5) `seen-cart-fab-tip` cart-FAB → överför veckomeny (meny, target, efter templates); (6) `seen-list-actions-tip` 3-prickar i inköpslistan (inköp, target); (7) `seen-sort-tip` sortera-knappen i recept (recept, target).
