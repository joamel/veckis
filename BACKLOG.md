...
- [~] Refaktorera och skapa fler filer för egna komponenter mm (separat från legacy-städningen ovan — bryta ut komponenter ur de stora skärmfilerna). **Påbörjad** (branch `refactor/extract-components`): `MultiMemberPicker` utbruten ur chores.tsx → egen fil med egen StyleSheet + 8 render-tester. Kvar: performer-pickern, SpotlightTip-gate m.fl.
