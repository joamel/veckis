...
- [x] Butiks-ikon fäster i navbaren vid scroll: butiksknappen är första scroll-raden och försvinner upp. Nu tonas en `storefront`-ikon in i navbaren (vänster, efter bakåt-knappen) via `scrollY`-interpolation så man fortsatt ser vilken butik listan gäller.
- [ ] Sticky kategori-rubrik: aktuell kategorirubrik ska fastna överst i inköpslistan tills man scrollar till nästa kategori. Kräver omstrukturering av header/scroll-arkitekturen (kollapsande header är absoluta overlays ovanpå en helskärms-ScrollView → `stickyHeaderIndices` fastnar bakom navbaren). Alternativ: en manuell pinnad "aktuell kategori"-overlay som uppdateras från scroll-offset + per-grupp `onLayout`.
