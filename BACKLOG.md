...
- [x] Onboarding-tips för dolda funktioner via `useOnceFlag`-hook + ny `SpotlightTip`-komponent: dimmar bakgrunden och ringer in mål-elementet med en pulserande ram (eller centrerad ruta om inget mål). Tips visas en gång per device (flagga per moment). Klart hittills: (1) `seen-forgiving-tip` — förfallna sysslor påminner inte mer; (2) `seen-menu-nav-tip` — dra rätter mellan dagar + svep mellan veckor; (3) `seen-merge-tip` — dubblett-knappen spotlightas första gången den poppar upp; (4) `seen-templates-tip` — veckomeny-mallar-ikonen spotlightas, visas EFTER nav-tipset så användaren inte bombas. Tips serialiseras via seen-flaggor.
