...
- [ ] Distribution: erbjuda nedladdning av Android-APK (eas-build artifact eller egen download-sida) och tydlig "Lägg till på hemskärmen / installera PWA"-instruktion (Chrome auto-prompt, iOS Safari manuell). En enkel landningssida som detekterar browser/OS och visar rätt instruktion. iOS App Store-publicering är fortfarande beroende av Apple Dev-konto + TestFlight — APK + PWA är "gratis-vägen".
- [ ] Snäva CORS från `*` till en faktisk whitelist när vi distribuerar bredare. Koden i `backend/src/index.ts` stöder redan komma-separerad `CORS_ORIGIN` env var; behöver bara sättas på Render. Förra försöket (`https://veckis-web.onrender.com,http://localhost:3000,...`) gav inga ACAO-headers — gör koden mer förlåtande först (lowercase + strip trailing slash + log `[CORS] Blocked origin: X` när matchning failar) så framtida typos blir synliga i Render logs istället för silent fail.
