...
- [ ] Backend integration-tester — Prisma-routes testas idag inte alls (bara rena helpers). Sätt upp vitest med en isolerad test-DB (tx-rollback per test) och täck minst: create/update chore med assignedToMany + rotation; medlem-borttagning rensar arrays; notification-scheduler end-to-end (fan-out till rätt clerk-users); shopping list merge + dedupe. Förhindrar regression när vi mexar med kritiska flöden.
