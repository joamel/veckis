...
- [x] Onboarding-tip för förlåtande-modellen: visas första gången användaren ser en återkommande syssla i 'förfallen'-status — `ConfirmDialog`-tip via ny `useOnceFlag('seen-forgiving-tip')`-hook (expo-secure-store), märks som sedd vid 'Förstått'-tryck. Mönstret återanvänds för framtida tips: en flagga per onboarding-moment.
